...
// ─── AI PROMPTS ───────────────────────────────────────────────────────────────
const AI_PROMPTS = {
  "0.1": (d, n) => `Sei un esperto SEO specializzato in siti WordPress per business locali italiani.

Il cliente è: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città principale: ${d.citta || "[INSERISCI CITTÀ]"}
Keyword concordate con il cliente:
- Keyword 1 (principale, per la Home): ${d.keyword1 || "[INSERISCI KEYWORD 1]"}
- Keyword 2 (seconda pagina SEO): ${d.keyword2 || "[INSERISCI KEYWORD 2]"}
- Keyword 3 (terza pagina SEO): ${d.keyword3 || "[INSERISCI KEYWORD 3]"}

Genera l'architettura completa del sito con:
1. Struttura URL definitiva per tutte le pagine (Home, Servizi, pagine SEO, Chi siamo, Contatti)
2. Per ogni pagina: URL slug, keyword principale, keyword secondarie, intento di ricerca, CTA principale
3. Note su eventuali pagine aggiuntive consigliate per questo settore

Formato output: tabella con colonne Pagina | URL | Keyword principale | Keyword secondarie | Intento | CTA`,

  "0.2": (d, n) => `Sei un esperto SEO specializzato in business locali italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Keyword principale (Home): ${d.keyword1 || "[INSERISCI KEYWORD 1]"}
Keyword pagina 2: ${d.keyword2 || "[INSERISCI KEYWORD 2]"}
Keyword pagina 3: ${d.keyword3 || "[INSERISCI KEYWORD 3]"}

Per ogni keyword crea una mappa completa con:
1. Keyword principale esatta da usare (verifica volume e intento)
2. 5 keyword secondarie correlate da distribuire nella pagina
3. Intento di ricerca (informazionale / commerciale / transazionale)
4. Domande correlate che i potenziali clienti fanno su Google
5. CTA principale consigliata per quella pagina

Formato output: una sezione per ogni keyword, strutturata e pronta da usare come riferimento durante la costruzione del sito.`,

  "4.1": (d, n) => `Sei un copywriter esperto in conversioni per siti web di business locali italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Servizio principale: ${d.keyword1 || "[INSERISCI SERVIZIO/KEYWORD PRINCIPALE]"}
Numero di telefono: ${d.telefono || "[INSERISCI NUMERO]"}

Genera 5 varianti di testo per la CTA nell'header del sito.
Ogni variante deve essere:
- Max 4 parole
- Orientata all'azione immediata
- Adatta al settore
- Diversa per tono (urgente, professionale, amichevole, convenienza, disponibilità)

Per ogni variante indica anche il sotto-testo opzionale (max 8 parole) da affiancare al bottone.`,

  "4.2": (d, n) => `Sei un copywriter specializzato in brand identity per PMI italiane.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Anni di attività: ${d.anniAttivita || "[INSERISCI ANNI]"}
Punti di forza: ${d.puntiForza || "[INSERISCI PUNTI DI FORZA]"}

Scrivi una descrizione aziendale per il footer del sito.
Requisiti:
- Max 300 caratteri (spazi inclusi)
- Deve contenere la keyword: ${d.keyword1 || "[KEYWORD PRINCIPALE]"}
- Tono professionale ma umano
- Deve trasmettere fiducia e competenza locale
- NON iniziare con il nome dell'azienda

Fornisci 3 varianti tra cui scegliere.`,

  "4.3": (d, n) => `Sei un copywriter UX specializzato in micro-copy per siti web italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Tono del brand: ${d.tonoBrand || "Amichevole"}

Scrivi il testo completo per la pagina 404 personalizzata del sito.
Deve includere:
1. Titolo principale (max 6 parole, tono coerente col brand)
2. Sottotitolo esplicativo (max 15 parole, spiega cosa è successo)
3. Testo body (2 righe max, rassicura l'utente e suggerisce cosa fare)
4. Testo del bottone "Torna alla Home" (max 4 parole, alternativo al generico)
5. Testo link secondario verso la pagina Contatti (max 5 parole)

Fornisci 2 varianti complete: una formale e una più leggera.`,

  "4.5": (d, n) => `Sei un copywriter SEO esperto in siti web per business locali italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Pagina da scrivere: [SPECIFICA QUALE PAGINA - es. Home / Pagina servizio / Chi siamo]
Keyword principale: ${d.keyword1 || "[INSERISCI KEYWORD]"}
Keyword secondarie: [INSERISCI 3-4 KEYWORD SECONDARIE]
Intento di ricerca: [INFORMAZIONALE / COMMERCIALE / TRANSAZIONALE]
CTA principale: ${d.ctaPrincipale || "[ES. Chiama ora / Richiedi preventivo]"}
Punti di forza del business: ${d.puntiForza || "[INSERISCI PUNTI DI FORZA]"}

Scrivi i testi completi per questa pagina:
1. H1 (max 60 caratteri, contiene keyword principale)
2. Paragrafo introduttivo (3-4 righe, keyword principale entro le prime 100 parole)
3. Sezione servizi/vantaggi: 3 sottotitoli H2 con keyword secondarie + 2-3 righe di testo ciascuno
4. Sezione prova sociale: 1 frase su esperienza/clienti soddisfatti
5. CTA finale: titolo H2 + frase di chiusura + testo bottone

Il testo deve essere naturale, non sovra-ottimizzato, orientato alla conversione.`,

  "4.6": (d, n) => `Sei un copywriter specializzato in UX writing per siti web italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Tempo di risposta indicativo: [ES. entro 24h / stesso giorno]
Canale di risposta preferito: [EMAIL / TELEFONO / ENTRAMBI]
Telefono: ${d.telefono || "[INSERISCI NUMERO]"}

Scrivi il messaggio di ringraziamento che appare dopo l'invio del form di contatto.
Deve includere:
1. Conferma dell'invio riuscito
2. Aspettativa chiara sui tempi di risposta
3. Eventuale azione alternativa se urgente
4. Tono caldo e professionale

Requisiti:
- Max 3 righe totali
- Niente esclamativi eccessivi
- Deve far sentire l'utente in buone mani

Fornisci 3 varianti: breve, media, completa.`,

  "5.1": (d, n) => `Sei un esperto SEO specializzato in ottimizzazione URL per siti WordPress italiani.

Per ognuna delle seguenti pagine genera lo slug URL ottimizzato:

Pagina Home — keyword: ${d.keyword1 || "[KEYWORD 1]"}
Pagina Servizi — keyword: servizi ${d.settore || "[SETTORE]"}
Pagina SEO 2 — keyword: ${d.keyword2 || "[KEYWORD 2]"}
Pagina SEO 3 — keyword: ${d.keyword3 || "[KEYWORD 3]"}
Chi siamo
Contatti
Città: ${d.citta || "[CITTÀ]"}

Per ogni slug rispetta:
- Solo lettere minuscole, parole separate da trattini
- Max 4-5 parole, contiene la keyword principale
- Niente articoli o preposizioni (il, della, per, ecc.)
- Niente caratteri accentati (usa e non è, a non à)

Fornisci per ogni pagina: slug consigliato + 1 alternativa + motivazione.`,

  "5.2": (d, n) => `Sei un esperto SEO on-page specializzato in siti WordPress italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Pagina: [SPECIFICA QUALE PAGINA]
Keyword principale (H1): ${d.keyword1 || "[KEYWORD PRINCIPALE]"}
Keyword secondarie disponibili: ${d.keyword2 || "[KW2]"}, ${d.keyword3 || "[KW3]"}, [AGGIUNGI ALTRE]
Struttura pagina prevista: Hero, Servizi, Perché sceglierci, Recensioni, CTA

Genera la struttura heading completa per questa pagina:
- 1 solo H1 con keyword principale (max 60 caratteri)
- H2 per ogni sezione principale (contengono keyword secondarie)
- H3 per eventuali sotto-punti degli H2

Per ogni heading indica: testo esatto, tag corretto (H1/H2/H3), keyword inclusa, sezione di appartenenza.`,

  "5.3": (d, n) => `Sei un esperto SEO specializzato in ottimizzazione title tag per siti italiani.

Nome business: ${n}
Città: ${d.citta || "[INSERISCI CITTÀ]"}

Pagina Home — keyword: ${d.keyword1 || "[KEYWORD]"}
Pagina Servizi — keyword: servizi ${d.settore || "[SETTORE]"}
Pagina ${d.keyword2 || "SEO 2"} — keyword: ${d.keyword2 || "[KEYWORD]"}
Pagina ${d.keyword3 || "SEO 3"} — keyword: ${d.keyword3 || "[KEYWORD]"}
Chi siamo — keyword: ${n} ${d.citta || ""}
Contatti — keyword: contatta ${n}

Per ogni title:
- Formato: Keyword Principale | Nome Brand
- Max 60 caratteri spazi inclusi
- Contiene la keyword principale e la città se rilevante
- NON duplicare lo stesso title su più pagine

Output: tabella con colonne Pagina | Title SEO | N° caratteri`,

  "5.4": (d, n) => `Sei un esperto SEO e copywriter specializzato in meta description per siti italiani.
...
Pagina Home — keyword: ${d.keyword1 || "[KEYWORD]"} — punto di forza: ${d.puntiForza ? d.puntiForza.split(/[,\n]/)[0] : "[ES. disponibile 24h]"}
Pagina Servizi — keyword: servizi ${d.settore || "[SETTORE]"} — punto di forza: [INSERISCI]
Pagina ${d.keyword2 || "SEO 2"} — keyword: ${d.keyword2 || "[KEYWORD]"} — punto di forza: [INSERISCI]
Pagina ${d.keyword3 || "SEO 3"} — keyword: ${d.keyword3 || "[KEYWORD]"} — punto di forza: [INSERISCI]
Chi siamo — keyword: ${n} ${d.citta || ""} — punto di forza: [INSERISCI]
Contatti — keyword: contatta ${n} — punto di forza: [INSERISCI]

Per ogni meta description:
- Struttura: [cosa fai] + [dove] + [vantaggio] + [CTA]
- Contiene la keyword principale, max 155 caratteri
- NON duplicare su più pagine

Output: tabella con colonne Pagina | Meta Description | N° caratteri`,

  "5.5": (d, n) => `Sei un esperto SEO tecnico specializzato in schema markup JSON-LD per business locali italiani.

DATI AZIENDALI:
Nome business: ${n}
Indirizzo: ${d.indirizzo || "[INSERISCI INDIRIZZO COMPLETO]"}
Telefono: ${d.telefono || "[INSERISCI TELEFONO]"}
Email: ${d.email || "[INSERISCI EMAIL]"}
Sito web: ${d.urlSito || "[INSERISCI URL]"}
Orari: ${d.orari || "[ES. Lun-Ven 9-18]"}
Settore/categoria: ${d.settore || "[INSERISCI SETTORE]"}
Servizi principali: ${d.keyword1 || "[SERVIZIO 1]"}, ${d.keyword2 || "[SERVIZIO 2]"}, ${d.keyword3 || "[SERVIZIO 3]"}
Social profiles: ${d.socialProfiles || "[INSERISCI URL SOCIAL]"}

Genera il codice JSON-LD completo e pronto da incollare per:
1. Schema LocalBusiness (per Home e pagina Contatti)
2. Schema Service (per la pagina ${d.keyword1 || "servizio principale"})
3. Schema Organization (per Home)

Per ogni schema indica anche dove inserirlo in Rank Math.`,

  "5.6": (d, n) => `Sei un esperto SEO specializzato in ottimizzazione immagini per siti WordPress italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Keyword principale: ${d.keyword1 || "[KEYWORD]"}

Ho queste immagini da caricare:
1. Foto dell'esterno del negozio/ufficio/cantiere
2. Foto del team al lavoro
3. Immagine hero della Homepage
4. Foto del servizio principale
5. Logo aziendale
[Aggiungi altre immagini se necessario]

Per ognuna genera:
- Nome file ottimizzato (solo minuscole, trattini, keyword rilevante, .webp)
- Alt text ottimizzato (descrittivo + keyword, max 125 caratteri)
- Title attribute opzionale

Output: tabella con colonne Immagine | Nome file | Alt text | Title`,

  "5.7": (d, n) => `Sei un esperto SEO specializzato in link building interno per siti WordPress italiani.
...
Pagine del sito:
- Home (keyword: ${d.keyword1 || "[KEYWORD]"})
- Servizi (keyword: servizi ${d.settore || "[SETTORE]"})
- ${d.keyword2 || "Pagina SEO 2"} (keyword: ${d.keyword2 || "[KEYWORD]"})
- ${d.keyword3 || "Pagina SEO 3"} (keyword: ${d.keyword3 || "[KEYWORD]"})
- Chi siamo
- Contatti

Per ogni collegamento interno suggerisci:
1. Pagina di partenza → Pagina di destinazione
2. Testo anchor ottimizzato (descrittivo, contiene keyword, max 5 parole)
3. Contesto suggerito (dove inserire il link nel testo)

Schema minimo: Home → tutte le pagine principali | Servizi → pagine SEO | Pagine SEO → si linkano a vicenda e a Contatti

Output: tabella con colonne Da | Verso | Anchor text | Contesto`,

  "7.5": (d, n) => `Sei un copywriter SEO specializzato in Google Business Profile per business locali italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città e zone servite: ${d.citta || "[CITTÀ]"}${d.zoneServite ? " + " + d.zoneServite : ""}
Anni di attività: ${d.anniAttivita || "[INSERISCI ANNI]"}
Servizi principali: ${d.keyword1 || "[SERVIZIO 1]"}, ${d.keyword2 || "[SERVIZIO 2]"}, ${d.keyword3 || "[SERVIZIO 3]"}
Punti di forza: ${d.puntiForza || "[INSERISCI PUNTI DI FORZA]"}
CTA principale: ${d.ctaPrincipale || "Contattaci oggi"}

Scrivi la descrizione per la scheda Google Business Profile.
Requisiti:
- Esattamente 750 caratteri (Google tronca oltre)
- Contiene le keyword principali in modo naturale
- Menziona la città e le zone servite
- NON usare emoji, NON usare superlativi vuoti
- Termina con una CTA

Fornisci 2 varianti: una istituzionale e una più diretta/commerciale.
Indica il conteggio caratteri preciso per ogni variante.`,

  "7.6": (d, n) => `Sei un copywriter specializzato in comunicazione clienti per PMI italiane.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Nome del referente: ${d.nomeReferente || "[NOME TITOLARE/REFERENTE]"}
Link recensione Google: [INCOLLA IL LINK DA GOOGLE BUSINESS PROFILE]

Scrivi i messaggi per richiedere una recensione Google ai clienti.

Fornisci:
1. Messaggio WhatsApp (max 3 righe, tono informale e diretto)
2. Email (oggetto + corpo max 5 righe, tono professionale ma caldo)
3. Versione breve per SMS (max 160 caratteri)

Per tutti:
- Il link recensione deve essere ben visibile
- NON deve sembrare automatizzato
- NON offrire incentivi (viola policy Google)
- Deve far sentire il cliente importante, non sollecitato`,

  "7.9": (d, n) => `Sei un esperto SEO e content strategist specializzato in blog per business locali italiani.

Cliente: ${n}
Settore: ${d.settore || "[INSERISCI SETTORE]"}
Città: ${d.citta || "[INSERISCI CITTÀ]"}
Keyword principali: ${d.keyword1 || "[KW1]"}, ${d.keyword2 || "[KW2]"}, ${d.keyword3 || "[KW3]"}
Target cliente: ${d.targetCliente || "[ES. privati con casa di proprietà / aziende / famiglie]"}

Crea un piano editoriale per i primi 3 mesi (6 articoli totali, 2 al mese).

Per ogni articolo:
1. Titolo ottimizzato SEO
2. Keyword target (primaria + 2 secondarie)
3. Intento di ricerca
4. Struttura heading suggerita (H1 + 3-4 H2)
5. Lunghezza consigliata (parole)
6. Priorità (1-6, dal più urgente)

Criteri: keyword a bassa/media concorrenza, ricerche che fanno i clienti PRIMA di contattare un professionista, articoli che si posizionano in 60-90 giorni.

Output: tabella + note strategiche.`,

  "M.3": (d, n) => `Sei un consulente SEO che prepara report mensili per clienti business locali italiani.

Cliente: ${n}
Mese di riferimento: [INSERISCI MESE/ANNO]
Mese precedente (confronto): [INSERISCI MESE/ANNO]

Dati GA4 del mese:
- Utenti: [NUMERO]
- Sessioni: [NUMERO]
- Utenti organici: [NUMERO]
- Pagina più visitata: [PAGINA] con [N] visualizzazioni
- Tasso di rimbalzo: [%]
- Durata media sessione: [TEMPO]
- Conversioni form: [NUMERO]
- Confronto utenti mese precedente: [+/- %]

Dati Search Console:
- Clic totali: [NUMERO] | Impressioni: [NUMERO] | CTR: [%] | Posizione media: [NUMERO]
- Keyword in crescita: [KEYWORD 1, KEYWORD 2]

Scrivi un report mensile da inviare al cliente.
Requisiti:
- Linguaggio semplice, zero tecnicismi, max 1 pagina
- Struttura: Sintesi del mese → Cosa ha funzionato → Cosa migliorare → Azioni del mese prossimo
- Tono professionale e ottimista ma onesto
- 3-5 dati chiave in evidenza, 2 azioni concrete per il mese successivo`,
};

function getPromptKey(taskTitle) {
  const map = {
    "0.1": "0.1", "0.2": "0.2",
    "4.1": "4.1", "4.2": "4.2", "4.3": "4.3", "4.5": "4.5", "4.6": "4.6",
    "5.1": "5.1", "5.2": "5.2", "5.3": "5.3", "5.4": "5.4", "5.5": "5.5", "5.6": "5.6", "5.7": "5.7",
    "7.5": "7.5", "7.6": "7.6", "7.9": "7.9",
    "M.3": "M.3",
  };
  for (const [prefix, key] of Object.entries(map)) {
    if (taskTitle.startsWith(prefix)) return key;
  }
  return null;
}
...
// ─── CHECKLIST PER DOMINIO ESISTENTE ──────────────────────────────────────────
const DEFAULT_CHECKLIST_EXISTING = [
  {
    phase: "FASE 0 — Analisi Sito Esistente",
    tasks: [
      {
        title: "0.1 — Analisi SEO del sito attuale",
        description: `Il cliente ha già un dominio attivo. Prima di toccare qualsiasi cosa, analizza la situazione attuale:\n• Quante pagine sono indicizzate su Google (site:dominio.it)\n• Posizioni attuali per le keyword principali\n• Backlink esistenti (non perderli!)\n• Errori tecnici presenti (404, redirect, ecc.)\n• Struttura URL attuale e storico di indicizzazione\nDOCUMENTA TUTTO prima di procedere.`,
        practicalGuide: `💡 UTILITÀ: Modificare un sito esistente senza analizzarlo prima può distruggere traffico organico già acquisito. Ogni pagina indicizzata ha valore — anche quelle brutte.\n\nPASSO PASSO PRATICO:\n1. Vai su Google e digita site:dominio.it — annota quante pagine sono indicizzate.\n2. Apri Google Search Console (se il cliente ce l'ha) e scarica le keyword con più clic.\n3. Usa Ubersuggest o Semrush (free) per vedere le keyword per cui si posiziona.\n4. Apri Screaming Frog (free fino a 500 URL) e crawla il sito: annota URL, title, meta, H1 di ogni pagina.\n5. Controlla i backlink con Ahrefs Backlink Checker free o Ubersuggest.\n6. Crea un documento con tutto: pagine indicizzate, keyword attuali, backlink principali, errori trovati.\n7. Solo dopo questa analisi decidi quali pagine mantenere, quali migliorare, quali eliminare con redirect.`,
      },
      {
        title: "0.2 — Pianificare le keyword e la nuova struttura",
        description: `Sulla base dell'analisi del task 0.1, pianifica:\n• Keyword principale per la Home (conferma o modifica rispetto all'attuale)\n• Keyword per le pagine SEO dedicate\n• Decide quali pagine esistenti mantenere, migliorare o sostituire\n• Pianifica i redirect 301 per le pagine che cambieranno URL\nNon cambiare URL già indicizzati senza redirect 301.`,
        practicalGuide: `💡 UTILITÀ: Su un sito esistente ogni cambio di URL richiede un redirect 301 per non perdere il posizionamento acquisito. Un redirect mancante equivale a buttare via mesi di SEO.\n\nPASSO PASSO PRATICO:\n1. Crea una tabella: URL attuale | Nuovo URL | Redirect 301 necessario (sì/no).\n2. Per ogni pagina esistente decidi: mantieni URL (ottimizzi solo contenuto) oppure cambi URL (aggiungi redirect).\n3. Annota le keyword nuove che vuoi targettizzare con la nuova struttura.\n4. Verifica che nessuna keyword importante già posizionata venga abbandonata senza alternativa.\n5. Condividi il piano col cliente prima di procedere — deve approvare le modifiche strutturali.`,
      },
      {
        title: "0.3 — Raccogliere materiali aggiornati",
        description: `Raccogli dal cliente tutti i materiali aggiornati:\n• Logo in SVG o PNG trasparente (se cambiato)\n• Foto aggiornate del business\n• Testi nuovi o aggiornamenti ai servizi\n• Nuovi numeri di telefono, indirizzo, email\n• Eventuali nuove certificazioni o punti di forza`,
        practicalGuide: `💡 UTILITÀ: Aggiornare un sito con materiali obsoleti non migliora nulla. Il cliente spesso ha nuove foto, nuovi servizi e una storia più ricca da raccontare rispetto a quando ha creato il sito originale.\n\nPASSO PASSO PRATICO:\n1. Manda al cliente una lista precisa di cosa ti serve (non "mandami i materiali").\n2. Specifica: logo in SVG o PNG con sfondo trasparente, minimo 500px.\n3. Chiedi foto recenti — no stock photography.\n4. Chiedi di confermare o aggiornare: orari, zona servita, servizi principali, prezzi se esposti.\n5. Organizza tutto in cartelle: /loghi /foto /testi /dati-aziendali.`,
      },
    ],
  },
  {
    phase: "FASE 1 — Configurazione sul Dominio Esistente",
    tasks: [
      {
        title: "1.1 — Verificare hosting e accessi",
        description: `Il dominio esiste già. Verifica:\n• Dove è registrato il dominio (Hostinger, Aruba, GoDaddy...)\n• Dove è l'hosting attuale e quali sono le credenziali\n• Se WordPress è già installato o c'è un altro CMS\n• Se ci sono email professionali già attive\nRaccogli TUTTE le credenziali prima di procedere.`,
        practicalGuide: `💡 UTILITÀ: Lavorare su un sito esistente senza avere tutti gli accessi significa fermarsi a metà lavoro quando scopri che manca una credenziale.\n\nPASSO PASSO PRATICO:\n1. Chiedi al cliente: accesso hosting (hPanel/cPanel), accesso WordPress (wp-admin), accesso al registrar del dominio, credenziali email.\n2. Salvale SUBITO nel password manager con etichetta "[nomecliente] – [servizio]".\n3. Verifica che ogni accesso funzioni davvero — non aspettare di averne bisogno.\n4. Se il sito è su un hosting scarso (Aruba, Register.it con piano base): valuta la migrazione su Hostinger prima di procedere.\n5. Controlla la scadenza del dominio — se scade entro 60 giorni, rinnova subito.`,
      },
      {
        title: "1.2 — Fare backup completo prima di toccare qualsiasi cosa",
        description: `Prima di modificare QUALSIASI cosa sul sito esistente, scarica un backup completo:\n• Database WordPress\n• File del sito (wp-content + wp-config)\nConserva il backup per almeno 30 giorni.`,
        practicalGuide: `💡 UTILITÀ: Su un sito esistente il backup pre-lavoro è l'assicurazione sulla vita. Se qualcosa va storto puoi ripristinare esattamente com'era prima.\n\nPASSO PASSO PRATICO:\n1. Installa UpdraftPlus se non presente → vai su Impostazioni → Backup Now.\n2. Scarica il backup completo (database + file) sul tuo PC.\n3. Conservalo in una cartella con la data: "backup-pre-lavori-DD-MM-YYYY".\n4. Se l'hosting lo permette, fai anche un backup manuale dall'hPanel/cPanel.\n5. Verifica che il backup sia completo aprendo il file ZIP e controllando che contenga database e wp-content.\n6. NON procedere con le modifiche finché il backup non è nelle tue mani.`,
      },
      {
        title: "1.3 — Valutare migrazione hosting (se necessario)",
        description: `Se il sito è su un hosting lento o con limitazioni:\n• Valuta migrazione su Hostinger Business\n• Hostinger offre migrazione gratuita assistita\n• Alternativa: usa il Migration Plugin di Hostinger\nSe l'hosting è già adeguato, salta questo task.`,
        practicalGuide: `💡 UTILITÀ: Un hosting lento vanifica tutto il lavoro SEO — inutile ottimizzare se il sito carica in 5 secondi per colpa del server.\n\nPASSO PASSO PRATICO:\n1. Testa la velocità attuale su pagespeed.web.dev — se il punteggio mobile è < 50 l'hosting potrebbe essere il problema.\n2. Se vuoi migrare su Hostinger: acquista il piano Business con il dominio esistente.\n3. Usa il Migration Plugin di Hostinger: Dashboard → Siti Web → Migra sito.\n4. Dopo la migrazione, aggiorna i DNS del dominio per puntare al nuovo hosting.\n5. Attendi la propagazione DNS (fino a 48h, spesso meno di 1h).\n6. Verifica che il sito funzioni sul nuovo hosting prima di cancellare quello vecchio.`,
      },
      {
        title: "1.4 — Verificare e rinnovare SSL",
        description: `Controlla che il certificato SSL sia valido e attivo.\nSe scaduto o assente: attivalo dall'hPanel o tramite il pannello hosting.`,
        practicalGuide: `💡 UTILITÀ: Un sito esistente con SSL scaduto perde traffico ogni giorno perché il browser mostra "Non sicuro" a tutti i visitatori.\n\nPASSO PASSO PRATICO:\n1. Apri il sito → controlla se il lucchetto HTTPS è presente nel browser.\n2. Se non c'è o è rotto: vai nell'hPanel → SSL → Gestisci → Installa/Rinnova Let's Encrypt.\n3. Attendi che lo stato diventi "Attivo" (1-5 minuti).\n4. Verifica su ssllabs.com/ssltest che il certificato sia valido e senza warning.\n5. Controlla che HTTPS sia forzato su tutte le URL (niente contenuto misto HTTP/HTTPS).`,
      },
      {
        title: "1.5 — Impostare staging",
        description: `Prima di modificare il sito live, crea una copia staging.\nSu Hostinger Business: hPanel → Staging → Crea staging.\nAlternativa: duplica il sito con un plugin (WPStagecoach, Duplicator).`,
        practicalGuide: `💡 UTILITÀ: Su un sito esistente e magari già indicizzato, fare modifiche direttamente sul live è rischioso. Lo staging permette di testare le modifiche senza impatto sul traffico reale.\n\nPASSO PASSO PRATICO:\n1. hPanel → Hosting → Gestisci → Staging → Crea staging.\n2. Attendi la copia (3-10 minuti).\n3. Tutte le modifiche delle prossime fasi vanno fatte PRIMA sullo staging.\n4. Solo dopo aver verificato che tutto funzioni: porta le modifiche sul live.\n5. Aggiungi un noindex allo staging per evitare che Google lo indicizzi (Rank Math → Impostazioni sito → Noindex staging).`,
      },
      {
        title: "1.6 — Impostare/verificare email professionale",
        description: `Verifica che l'email professionale (info@dominio.it) esista e funzioni.\nSe non esiste: creala dall'hPanel → Email → Gestisci → Crea account email.`,
        practicalGuide: `💡 UTILITÀ: I form del sito inviano notifiche via email — se l'email non funziona i lead spariscono nel nulla senza che nessuno se ne accorga.\n\nPASSO PASSO PRATICO:\n1. Vai su hPanel → Email → verifica che info@dominio.it esista.\n2. Se non esiste: Crea account email → info@dominio.it.\n3. Testa inviando una email di prova da Gmail.\n4. Verifica che il form del sito (se già presente) invii le notifiche all'email corretta.\n5. Aggiorna le impostazioni del form se necessario.`,
      },
    ],
  },
  { phase: "FASE 2 — Configurazione WordPress", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 2 — Configurazione WordPress")?.tasks || [] },
  { phase: "FASE 3 — Tema e Plugin", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 3 — Tema e Plugin")?.tasks || [] },
  { phase: "FASE 4 — Costruzione con Elementor Pro", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 4 — Costruzione con Elementor Pro")?.tasks || [] },
  { phase: "FASE 5 — Ottimizzazione SEO On-Page", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 5 — Ottimizzazione SEO On-Page")?.tasks || [] },
  { phase: "FASE 6 — Prima di Pubblicare", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 6 — Prima di Pubblicare")?.tasks || [] },
  { phase: "FASE 7 — Dopo la Pubblicazione", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "FASE 7 — Dopo la Pubblicazione")?.tasks || [] },
  { phase: "MANUTENZIONE MENSILE", tasks: DEFAULT_CHECKLIST.find(s => s.phase === "MANUTENZIONE MENSILE")?.tasks || [] },
];
...
function ProjectApp({ client, onBack }) {
  ...
  const defaultChecklist = client.projectType === "esistente" ? DEFAULT_CHECKLIST_EXISTING : DEFAULT_CHECKLIST;
  const [structure, setStructure] = useState(defaultChecklist);
  ...
  useEffect(() => {
    async function load() {
      ...
      let resolvedStructure = client.projectType === "esistente" ? DEFAULT_CHECKLIST_EXISTING : DEFAULT_CHECKLIST;
      let resolvedState = [];
      ...
      const ad = localStorage.getItem(`seo-aidata-${client.id}`);
      if (ad) { try { setAiData(JSON.parse(ad)); } catch {} }
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#fff", fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
                ...
                <span style={{ fontSize: 10, background: client.projectType === "esistente" ? "rgba(96,165,250,0.15)" : "rgba(167,139,250,0.12)", border: `1px solid ${client.projectType === "esistente" ? "rgba(96,165,250,0.3)" : "rgba(167,139,250,0.25)"}`, borderRadius: 20, padding: "1px 8px", color: client.projectType === "esistente" ? "#93c5fd" : "#c4b5fd", fontWeight: 600 }}>
                  {client.projectType === "esistente" ? "🔄 Dominio esistente" : "🆕 Dominio nuovo"}
                </span>
                <SyncDot />
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <button
                ...
                title="Dati cliente per i prompt AI"
                style={{
                    ...
                    <div
                      ...
                      {!editMode && isExpanded && (
                        <div style={{ padding: "0 14px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
                          ...
                          {/* AI Prompt button */}
                          {(() => {
                            const pKey = getPromptKey(task.title);
                            if (!pKey) return null;
                            return (
                              <button
                                onClick={() => {
                                  const prompt = AI_PROMPTS[pKey](aiData, client.name);
                                  navigator.clipboard.writeText(prompt).then(() => {
                                    setPromptToast(true);
                                    setTimeout(() => setPromptToast(false), 3000);
                                ...
                                🤖 Genera con AI — Copia prompt e apri ChatGPT
                              </button>
                            );
                          })()}
                          {/* Timer */}
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowAiModal(false)}>
          <div className="modal-box" style={{ maxWidth: 640 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <div>
                <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.2rem", fontWeight: 800, margin: 0 }}>🤖 Dati Cliente per i Prompt AI</h2>
                <p style={{ fontSize: 12, color: "#71717a", margin: "4px 0 0" }}>Questi dati vengono inseriti automaticamente nei prompt. Più campi compili, più i prompt saranno precisi.</p>
              </div>
              <button onClick={() => setShowAiModal(false)} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#a1a1aa", cursor: "pointer", fontSize: 18, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
            </div>

            {[
              { label: "Settore", key: "settore", placeholder: "es. Idraulico, Avvocato, Estetista", type: "text" },
              { label: "Città principale", key: "citta", placeholder: "es. Milano", type: "text" },
              { label: "Zone servite (opzionale)", key: "zoneServite", placeholder: "es. Milano, Monza, Sesto San Giovanni", type: "text" },
              { label: "Keyword 1 — Principale (Home)", key: "keyword1", placeholder: "es. idraulico milano", type: "text" },
              { label: "Keyword 2 — Pagina SEO 2", key: "keyword2", placeholder: "es. spurghi fognature milano", type: "text" },
              { label: "Keyword 3 — Pagina SEO 3", key: "keyword3", placeholder: "es. videoispezioni tubazioni", type: "text" },
              { label: "CTA principale", key: "ctaPrincipale", placeholder: "es. Chiama ora / Richiedi preventivo", type: "text" },
              { label: "Telefono", key: "telefono", placeholder: "es. 02 1234567", type: "text" },
              { label: "Email", key: "email", placeholder: "es. info@nomesito.it", type: "text" },
              { label: "URL sito", key: "urlSito", placeholder: "es. https://nomesito.it", type: "text" },
              { label: "Indirizzo completo", key: "indirizzo", placeholder: "es. Via Roma 1, 20100 Milano MI", type: "text" },
              { label: "Orari", key: "orari", placeholder: "es. Lun-Ven 9-18, Sab 9-13", type: "text" },
              { label: "Anni di attività", key: "anniAttivita", placeholder: "es. 15", type: "text" },
              { label: "Social profiles (URL)", key: "socialProfiles", placeholder: "es. facebook.com/nome, instagram.com/nome", type: "text" },
              { label: "Nome referente/titolare", key: "nomeReferente", placeholder: "es. Marco Rossi", type: "text" },
              { label: "Target cliente", key: "targetCliente", placeholder: "es. privati con casa di proprietà / aziende", type: "text" },
              ...
              <div key={key} style={{ marginBottom: 12 }}>
                ...
                <input
                  ...
                  onChange={(e) => setAiData((prev) => ({ ...prev, [key]: e.target.value }))}
                />
              </div>
            ))}

            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, color: "#71717a", fontWeight: 600, display: "block", marginBottom: 4 }}>Punti di forza</label>
              <textarea
                className="edit-textarea"
                rows={3}
                value={aiData.puntiForza || ""}
                placeholder="es. Disponibile 24h, intervento in 30 minuti, prezzi fissi senza sorprese"
                onChange={(e) => setAiData((prev) => ({ ...prev, puntiForza: e.target.value }))}
              />
            </div>

            <div style={{ marginBottom: 18 }}>
              <label style={{ fontSize: 11, color: "#71717a", fontWeight: 600, display: "block", marginBottom: 4 }}>Tono del brand</label>
              <select
                className="edit-input"
                value={aiData.tonoBrand || "Amichevole"}
                onChange={(e) => setAiData((prev) => ({ ...prev, tonoBrand: e.target.value }))}
              >
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button onClick={() => {
                localStorage.setItem(`seo-aidata-${client.id}`, JSON.stringify(aiData));
                setShowAiModal(false);
              }}
                style={{ padding: "8px 22px", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer", background: "rgba(251,191,36,0.2)", border: "1px solid rgba(251,191,36,0.4)", color: "#fcd34d" }}>
                💾 Salva
              </button>
            </div>
          </div>
      ...
      {promptToast && (
        <div style={{
          ...
          background: "#1a1a2e", border: "1px solid rgba(251,191,36,0.4)", borderRadius: 14,
          padding: "12px 20px", zIndex: 999, display: "flex", alignItems: "center", gap: 10,
          boxShadow: "0 8px 32px rgba(0,0,0,0.5)", animation: "fadeIn 0.2s ease",
        }}>
          <span style={{ fontSize: 18 }}>✅</span>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#fcd34d" }}>Prompt copiato negli appunti!</div>
            <div style={{ fontSize: 11, color: "#71717a" }}>ChatGPT si sta aprendo — incolla con Ctrl+V / ⌘V</div>
          </div>
        </div>
      )}
    </div>
...
function ClientList({ onSelect, taskStats = {}, setTaskStats = () => {} }) {
  ...
  const [newProjectType, setNewProjectType] = useState("nuovo"); // "nuovo" | "esistente"
    ...
    async function loadFromSupabase() {
      ...
      if (!error) {
        const mapped = (data || []).map((r) => ({
          ...
          projectType: r.project_type || "nuovo",
        }));
  ...
  const saveClientsRemote = async (list) => {
    ...
    for (const c of list) {
      const row = {
        ...
        project_type: c.projectType || "nuovo",
      };
  ...
  const addClient = async () => {
    ...
    localStorage.setItem(`seo-projecttype-${client.id}`, newProjectType);
    setNewName("");
    setNewStartDate("");
    setNewDeadline("");
    setNewProjectType("nuovo");
    setSaving(false);
  };
      ...
      {!loadingClients && <div style={{ maxWidth: 780, margin: "0 auto", padding: "32px 16px 60px" }}>
        ...
        <div style={{ marginBottom: 28, textAlign: "center" }}>
          ...
          <button onClick={async () => {
            ...
            if (!error) {
              const mapped = data.map((r) => ({ id: r.id, name: r.name, createdAt: r.created_at, publishedDate: r.published_date || null, deadline: r.deadline || null, startDate: r.start_date || null, projectType: r.project_type || "nuovo" }));
              saveClientsLocal(mapped);
            }
            setLoadingClients(false);
                  ...
                  <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => !editingId && onSelect(client)}>
                    {editingId === client.id ? (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: 10, background: client.projectType === "esistente" ? "rgba(96,165,250,0.15)" : "rgba(167,139,250,0.15)", border: `1px solid ${client.projectType === "esistente" ? "rgba(96,165,250,0.3)" : "rgba(167,139,250,0.3)"}`, borderRadius: 20, padding: "2px 8px", color: client.projectType === "esistente" ? "#93c5fd" : "#c4b5fd", fontWeight: 600 }}>
                          {client.projectType === "esistente" ? "🔄 Esistente" : "🆕 Nuovo"}
                        </span>
                        {alerts.map((a, i) => (
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "2px dashed rgba(167,139,250,0.2)", borderRadius: 16, padding: "18px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[
                { val: "nuovo", label: "🆕 Dominio nuovo", desc: "Acquisto dominio + hosting da zero" },
                { val: "esistente", label: "🔄 Dominio esistente", desc: "Sito già online da migliorare" },
              ].map(({ val, label, desc }) => (
                <button key={val} onClick={() => setNewProjectType(val)}
                  style={{
                    flex: 1, padding: "10px 12px", borderRadius: 10, fontSize: 12, fontWeight: 700,
                    cursor: "pointer", textAlign: "left", transition: "all 0.15s",
                    background: newProjectType === val ? (val === "nuovo" ? "rgba(167,139,250,0.2)" : "rgba(96,165,250,0.2)") : "rgba(255,255,255,0.03)",
                    border: `1px solid ${newProjectType === val ? (val === "nuovo" ? "rgba(167,139,250,0.5)" : "rgba(96,165,250,0.5)") : "rgba(255,255,255,0.08)"}`,
                    color: newProjectType === val ? (val === "nuovo" ? "#c4b5fd" : "#93c5fd") : "#52525b",
                  }}>
                  <div>{label}</div>
